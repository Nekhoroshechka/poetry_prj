...
def test_mask_account_card(customer_details):
    assert mask_account_card('Visa Platinum 7000792289606361') == 'Visa Platinum 7000 79** **** 6361'
    assert mask_account_card('Счет 35383033474447895560') == 'Счет **5560'


@pytest.mark.parametrize('customer_details, expected', [
    ('Maestro 1596837868705199', 'Maestro 1596 83** **** 5199'),
    ('Счет 64686473678894779589', 'Счет **9589'),
    ('MasterCard 7158300734726758', 'MasterCard 7158 30** **** 6758'),
    ('Счет 73654108430135874305', 'Счет **4305')
])
def test_mask_account_cards(customer_details, expected):
    assert mask_account_card(customer_details) == expected


def test_mask_account_card_invalid_card_or_account():
...
def test_get_data(enter_data):
    assert get_data('2024-03-11T02:26:18.671407') == '11.03.2024'


@pytest.mark.parametrize('enter_data, expected', [
...
def test_get_data_(enter_data, expected):
